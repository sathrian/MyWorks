...
def GetArgs():
    ...
    parser.add_argument('-s', '--host', required=False, action='store',default="10.124.128.13",
...
def printHostInformation(host):
    try:
        ...
        cpuUsagePercentage = int(cpuUsage/(summary.hardware.cpuMhz*summary.hardware.numCpuCores)*100)
        memoryCapacity = hardware.memorySize
        memoryCapacityInMB = hardware.memorySize/MBFACTOR
        memoryUsage = stats.overallMemoryUsage
        MemoryPercentage = (float(memoryUsage) / memoryCapacityInMB) * 100
        Records.update({HostName:{'IP Address':vinc.spec.ip.ipAddress,
...
def printVmInformation(virtual_machine, depth=1):
    ...
    try:
        ...
        print("CPU Usage: ", summary.quickStats.overallCpuUsage/(summary.config.numCpu*2100))
        print("State : ", summary.runtime.powerState)
        hardware = summary.vm.config.hardware.device
        for d in hardware:
