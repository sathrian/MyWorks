        ...
        with open(LogFile , 'r', encoding='latin1') as infile:
            ...
            for line in infile:
              try:
                ...
                if Match:
                    ...
                    if mip in IP_list.keys():
                        if IP_list[mip]['update']:
                            if IP_list[ip]['reboot']['type']['details']['update']:
                                logging.info('Details first loop' )
                                if len(IP_list[mip].keys()) == 8:
                                    if len(IP_list[mip]['reboot']['type']['details'].keys()) != 6:
                                        logging.error('Details Missing for first loop: %s - %s'% (mip,IP_list[mip]['reboot']['type']['details'].keys()))
                            ...
                            t = t.replace(':','-')
                            logging.info(t)
                            filename = 'RebootLog_'+os.path.splitext(os.path.basename(LogFile))[0]+'_'+mip.replace(".", "_")+'_'+t+'.txt'
                        ...
                        IP_list[mip].update({'Time':Time,'RebootDetected':True, 'data':[line],'update':False,'reboot':{'type':{'details':{'update':False}}}})
                    else:
                        IP_list.update({mip:{'Time':Time,'RebootDetected':True, 'data':[line],'update':False,'reboot':{'type':{'details':{'update':False}}}}})
                    logging.info(line)
                else:
                    ...
                    if m:
                        ...
                        if IP_list[Ip]['RebootDetected']:
                            ...
                            IP_list[ip]['reboot']['type']['details']['update'] = True
                            RebootsFound += 1
                            continue
                        else:
                        ...
                        if ip in IP_list.keys():
                            if IP_list[ip]['RebootDetected']:
                                ...
                                if not 'Cause' in IP_list[ip]['reboot']['type']['details'].keys():
                                    Cause = re.search(r'Cause:\s*(.*)',pmtv, re.S|re.I)
                                    if Cause:
                            ...
                            if len(IP_list[ip].keys()) == 8 and len(IP_list[ip]['reboot']['type']['details'].keys()) == 6:
                                if IP_list[ip]['reboot']['type']['details']['update']:
                                    ...
                                    IP_list[ip]['reboot']['type']['details']['update'] = False
                ...
                logging.error(traceback.format_exc(6))
        write_all_to_file(IP_list, LogFile)
        for ip in IP_list.keys():
            if len(IP_list[ip].keys()) == 8:
                if IP_list[ip]['reboot']['type']['details']['update']:
                    if len(IP_list[ip]['reboot']['type']['details'].keys()) != 6:
                        missing_keys(IP_list, ip)
                        logging.error('Details Missing for at the end : %s - %s'% (ip,IP_list[ip]['reboot']['type']['details']))
                    write_csv(IP_list, ip, LogFile, model)
    ...
    try:
        ...
        if len(IP_list[ip].keys()) == 8:
            for reason in IP_list[ip]['reboot']['type'].keys():
                if reason == 'backtrace' or reason == 'data' or reason == 'details' or reason == 'Time' or reason == 'update':
                    continue
                if model == 'All':
...
if __name__=="__main__":
    ...
    CsvData = [['Date/Time','Reboot Reason', 'IP Address', 'Mac Address', 'HW Model', 'Version', 'PC', 'Back Trace', 'Thread', 'Cause', 'Up Time']]
    main(file_name)
