...
def Compile(ChooseCombo, Make=None):
    ...
    if os.path.exists(path):
        out1 = cmdexe("source %s; choosecombo %s"%(path, ChooseCombo))
        logging.info("choose combo -%s"%out1)
        if Make:
            out = cmdexe("make %s"%Make)
        else:
            out = cmdexe("make")
        logging.info("Make output:\r\n%s"%out)

        if 'The operation has completed successfully' in out:
